...
static bool moveLinearRelativeCm(float delta_cm) {
  ...
  while (millis() - t0 < 30000) {
    if (deltaSteps > 0 && rightPressed()) { lin.stop(); bncOff(BNC_LINEAR); return true; }
    if (deltaSteps < 0 && leftPressed())  { lin.stop(); bncOff(BNC_LINEAR); return true; }
...
static bool runRotary(float speed_cm_s, int8_t dir) {
  ...
  if (speed_cm_s <= 0.0f) {
    bncOff(BNC_ROTARY);
    disableMotor(EN_A);
    delay((uint32_t)lroundf(cfg.rot_duration_s * 1000.0f));
    return true;
  }
...
static bool doHomingAndSetZero() {
  ...
  bool ok = homeFastToSwitch(false);
  if (ok) lin.setHome();
  disableMotor(EN_B);
  return ok;
}
...
void loop() {
  ...
  switch (state) {
    ...
    case DO_HOMING: {
      oledStage("HOMING...", "LEFT only", "switch click -> 0");
